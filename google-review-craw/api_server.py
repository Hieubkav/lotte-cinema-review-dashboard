...
async def lifespan(app: FastAPI):
    ...
    # Raise parallelism for faster full-sync from homepage Sync button.
    job_manager = JobManager(max_concurrent_jobs=4)
    ...
    # Start daily scheduled scraper at 04:30 VN time (UTC+7)
...
async def scheduled_scraper():
    """
    Background task that triggers the scraper every day at 04:30 AM Vietnam Time (UTC+7)
    """
    log.info("Scheduled scraper task initialized (Target: 04:30 VN Time)")
    ...
    while True:
        ...
        # Check if it's 04:30
        if now_vn.hour == 4 and now_vn.minute == 30:
            log.info(f"Scheduled time reached (04:30 VN). Triggering bulk scrape...")
            try:
                # We reuse the logic from the /trigger-all endpoint
                from modules.config import load_config
                current_config = load_config()
                businesses = current_config.get("businesses", [])
                
                if businesses:
                    count = 0
                    for biz in businesses:
                        url = biz if isinstance(biz, str) else biz.get("url")
                        overrides = {} if isinstance(biz, str) else {k: v for k, v in biz.items() if k != "url"}
                        if url and job_manager:
                            job_id = job_manager.create_job(url, overrides)
                            job_manager.start_job(job_id)
                            count += 1
                    log.info(f"Scheduled task: triggered {count} jobs successfully.")
                else:
                    log.warning("Scheduled task: No businesses found in config.")
            except Exception as e:
                log.error(f"Error in scheduled task: {e}")
            
            # Sleep for 61 seconds to ensure we don't trigger multiple times in the same minute
...
@jobs_router.get("/trigger-all", summary="Trigger All Configured Scrapes")
async def trigger_all_jobs(background_tasks: BackgroundTasks):
    """
    Trigger scraping jobs for all businesses defined in config.yaml.
    """
    if not job_manager:
        raise HTTPException(status_code=500, detail="Job manager not initialized")
    ...
    if not businesses:
        return {"message": "No businesses found in config.yaml"}
    ...
    for biz in businesses:
        if isinstance(biz, str):
            ...
            url = biz.get("url")
            overrides = {k: v for k, v in biz.items() if k != "url"}
    ...
    return {
        ...
        "details": results,
        "message": f"Successfully triggered {len(results)} jobs from config."
    }
