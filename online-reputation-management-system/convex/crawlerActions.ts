...
import { fetchRealCrawl } from "../src/server/crawler/realSource";
import { normalizeReviews } from "../src/server/crawler/normalize";

function nowIso() {
  return new Date().toISOString();
}

function buildMetrics(reviews: Array<{ rating: number; isoDate?: string }>, officialAvgRating: number, officialTotalReviews: number) {
  ...
  const threshold = Date.now() - 30 * 24 * 3600_000;
  const reviewsLast30d = reviews.filter((review) => {
    if (!review.isoDate) return false;
    const ts = new Date(review.isoDate).getTime();
  ...
  return {
    metrics: {
      ...
      sentimentScore,
      density30d: Number((reviewsLast30d / 30).toFixed(3)),
      reviewsLast30d,
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(addEvent, {
        jobId,
        level: "info",
        message: "Đang fetch dữ liệu thật từ Google Maps",
      } as any);

      const rawCrawl = await fetchRealCrawl(args.url);
      const reviews = normalizeReviews(rawCrawl.reviews);
      const crawl = {
        officialTotalReviews: rawCrawl.officialTotalReviews,
        officialAvgRating: rawCrawl.officialAvgRating,
        reviews,
        ...buildMetrics(reviews, rawCrawl.officialAvgRating, rawCrawl.officialTotalReviews),
      };
